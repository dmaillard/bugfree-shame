 ...
 * @param <type> $results
 *
 * All custom options we want stored in WP and displayed on the ad detail page need to begin with _ajax_
...
function cp_image_input_fields() {
	...
	echo do_shortcode("[cp_ajax_uploader afterUpload='image' multi='true' startOn='auto' allowRemove='true']");
...
if ( !function_exists('cp_show_default_form') ) {
	function cp_show_default_form($renew = false) {
		...
		if ( $results ) {
			...
			echo  __( 'ERROR: no results found for the default ad form.', APP_TD ) . "\n\n" ;
...
if ( !function_exists('cp_calc_membership_cost') ) {
